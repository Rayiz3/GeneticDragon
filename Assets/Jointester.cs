...
public class Jointester : MonoBehaviour {
    ...
    void Start()
    {
        for (int i = 0; i < 48; i++)
        {
            gene[i] = UnityEngine.Random.Range(-90, 90);
        }
    ...
    {
        if(other.name != "Terrain" && other.name != "startline") Debug.Log(this.gameObject.name + "<-->" + other.gameObject.name);
    }
